	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float t = _Time * 20;
				float an = t*.5;
				float2 r = float2(p.x*cos(an) - p.y*sin(an), p.x*sin(an) + p.y*cos(an)), // rotate image clockwise
				uv = float2(.25*r.x / abs(r.y), 0.5*t + .01 / abs(r.y));	// change x coordinate according to y coordinate
				uv = frac(uv);
